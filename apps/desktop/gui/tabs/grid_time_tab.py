...
class GridTimeTab(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._timezone_label = QLabel('Timezone: local scenario time (Marshall: Mountain Standard Time, UTC−07:00)')
    ...
    def _update_steps_label(self, *_) -> None:
        ...
        if steps <= 0:
            ...
            self._steps_label.setText(f'  Calculated steps: {steps}  ({delta_h:.1f} h  ·  {_T_STEP_MIN:.0f}-min timestep  ·  grid = {self._grid_size.value():g} m)')
            self._steps_label.setStyleSheet('color: #555; font-style: italic;')

    def get_params(self) -> dict:
        return {'t_start': self._t_start.value(), 't_end': self._t_end.value(), 'maxstep': _calc_steps(self._t_start.value(), self._t_end.value()), 'grid_size': self._grid_size.value()}
